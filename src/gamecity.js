...
gltf.load('models/cities/trailer_park.glb', (g) => {
  ...
  // The trailer park has one intended start: the paved aisle dead-centre between
  // the six trailers. Spawn on the ground beneath that point. Only if that ray
  // somehow misses the ground do we fall back to the nearest open flat spot —
  // flatness alone isn't enough (trees + trailers sit on the same ground), so the
  // fallback also demands headroom + horizontal clearance so you don't wake up
  // buried in a tree.
  const AISLE = { x: 16, z: 20 };
  let spawn = null;
  const aisleY = groundY(AISLE.x, AISLE.z, top);
  if (aisleY != null) spawn = { x: AISLE.x, z: AISLE.z, y: aisleY };
  if (!spawn) {
    ...
    spawn = byCentre.find(isOpen) || byCentre[0] || { x: c.x, z: c.z, y: level };
  }
